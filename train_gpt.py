...
def main() -> None:
    ...
    if args.ema_decay > 0:
        for name, p in base_model.named_parameters():
            ema_state[name] = p.data.detach().clone()
    ...
    while True:
        ...
        if ema_state:
            with torch.no_grad():
                for name, p in base_model.named_parameters():
                    ema_state[name].mul_(args.ema_decay).add_(p.data, alpha=1 - args.ema_decay)
    ...
    if args.val_sliding_stride > 0 and args.val_sliding_stride < args.train_seq_len:
        log0(f"final_eval_mode:sliding_window stride:{args.val_sliding_stride}")
    ...
    torch.cuda.synchronize()
    log0(
        f"final_{compress_label}_roundtrip val_loss:{q_val_loss:.4f} val_bpb:{q_val_bpb:.4f} "
